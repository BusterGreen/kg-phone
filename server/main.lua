...
local function GetOnlineStatus(number)
    local Target = KGCore.Functions.GetPlayerByPhone(number)
    local retval = false
    if Target ~= nil then
        retval = true
    end
...
function KGPhone.AddMentionedTweet(citizenid, TweetData)
...
function KGPhone.SetPhoneAlerts(citizenid, app, alerts)
...
local function GenerateOwnerName()
    local names = {
        ...
        [15] = { name = 'Jacques Schmitt', citizenid = 'KGC091H93' },
...
local function sendNewMailToOffline(citizenid, mailData)
    ...
    if Player then
        ...
        if mailData.button == nil then
            ...
            TriggerClientEvent('kg-phone:client:NewMailNotify', src, mailData)
        else
        ...
        SetTimeout(200, function()
            ...
            TriggerClientEvent('kg-phone:client:UpdateMails', src, mails)
        end)
    else
        if mailData.button == nil then
...
KGCore.Functions.CreateCallback('kg-phone:server:GetCallState', function(_, cb, ContactData)
    local Target = KGCore.Functions.GetPlayerByPhone(ContactData.number)
    if Target ~= nil then
...
KGCore.Functions.CreateCallback('kg-phone:server:GetPhoneData', function(source, cb)
    local src = source
    ...
    if Player ~= nil then
        ...
        if invoices[1] ~= nil then
            for _, v in pairs(invoices) do
                local Ply = KGCore.Functions.GetPlayerByCitizenId(v.sender)
                if Ply ~= nil then
...
KGCore.Functions.CreateCallback('kg-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Invoices = {}
    local Ply = KGCore.Functions.GetPlayer(source)
    local SenderPly = KGCore.Functions.GetPlayerByCitizenId(sendercitizenid)
    ...
    exports['kg-phone']:sendNewMailToOffline(sendercitizenid, invoiceMailData)
    exports['kg-banking']:AddMoney(society, amount, 'Phone invoice')
...
KGCore.Functions.CreateCallback('kg-phone:server:DeclineInvoice', function(source, cb, _, _, invoiceId)
    local Invoices = {}
    local Ply = KGCore.Functions.GetPlayer(source)
    MySQL.query('DELETE FROM phone_invoices WHERE id = ?', { invoiceId })
    local invoices = MySQL.query.await('SELECT * FROM phone_invoices WHERE citizenid = ?', { Ply.PlayerData.citizenid })
...
KGCore.Functions.CreateCallback('kg-phone:server:GetContactPictures', function(_, cb, Chats)
...
KGCore.Functions.CreateCallback('kg-phone:server:GetContactPicture', function(_, cb, Chat)
...
KGCore.Functions.CreateCallback('kg-phone:server:GetPicture', function(_, cb, number)
    local query = '%' .. number .. '%'
    local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
    if result[1] ~= nil then
...
KGCore.Functions.CreateCallback('kg-phone:server:FetchResult', function(_, cb, search)
...
KGCore.Functions.CreateCallback('kg-phone:server:GetVehicleSearchResults', function(_, cb, search)
    search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
    ...
    if result[1] ~= nil then
        for k, _ in pairs(result) do
            ...
            if player[1] ~= nil then
                ...
                local vehicleInfo = KGCore.Shared.Vehicles[result[k].vehicle]
...
KGCore.Functions.CreateCallback('kg-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData
    if plate ~= nil then
        ...
        TriggerClientEvent('KGCore:Notify', src, 'No Vehicle Nearby', 'error')
        cb(nil)
    end
end)

KGCore.Functions.CreateCallback('kg-phone:server:HasPhone', function(source, cb)
...
KGCore.Functions.CreateCallback('kg-phone:server:CanTransferMoney', function(source, cb, amount, iban)
    ...
    if (Player.PlayerData.money.bank - amount) >= 0 then
        ...
        local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
        if result[1] ~= nil then
            local Reciever = KGCore.Functions.GetPlayerByCitizenId(result[1].citizenid)
...
KGCore.Functions.CreateCallback('kg-phone:server:GetCurrentLawyers', function(_, cb)
    local Lawyers = {}
...
KGCore.Functions.CreateCallback('kg-phone:server:GetWebhook', function(_, cb)
    if WebHook ~= '' then
        cb(WebHook)
    else
...
RegisterNetEvent('kg-phone:server:AddAdvert', function(msg, url)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('kg-phone:client:UpdateAdverts', -1, Adverts, '@' .. Player.PlayerData.charinfo.firstname .. '' .. Player.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('kg-phone:server:DeleteAdvert', function()
    local Player = KGCore.Functions.GetPlayer(source)
    local citizenid = Player.PlayerData.citizenid
    Adverts[citizenid] = nil
    TriggerClientEvent('kg-phone:client:UpdateAdvertsDel', -1, Adverts)
end)

RegisterNetEvent('kg-phone:server:SetCallState', function(bool)
    local src = source
...
RegisterNetEvent('kg-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
...
RegisterNetEvent('kg-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', { Player.PlayerData.citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0 })
...
RegisterNetEvent('kg-phone:server:sendNewEventMail', function(citizenid, mailData)
    local Player = KGCore.Functions.GetPlayerByCitizenId(citizenid)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', { citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0 })
    else
    ...
    SetTimeout(200, function()
        ...
        TriggerClientEvent('kg-phone:client:UpdateMails', Player.PlayerData.source, mails)
    end)
end)

RegisterNetEvent('kg-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
...
RegisterNetEvent('kg-phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for _, v in pairs(KGCore.Functions.GetPlayers()) do
        local Player = KGCore.Functions.GetPlayer(v)
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                KGPhone.SetPhoneAlerts(Player.PlayerData.citizenid, 'twitter')
                KGPhone.AddMentionedTweet(Player.PlayerData.citizenid, TweetMessage)
                TriggerClientEvent('kg-phone:client:GetMentioned', Player.PlayerData.source, TweetMessage, AppAlerts[Player.PlayerData.citizenid]['twitter'])
            else
                ...
                if result[1] ~= nil then
                    ...
                    KGPhone.SetPhoneAlerts(MentionedTarget, 'twitter')
                    KGPhone.AddMentionedTweet(MentionedTarget, TweetMessage)
                end
            end
        end
    end
...
RegisterNetEvent('kg-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = KGCore.Functions.GetPlayer(src)
    local Target = KGCore.Functions.GetPlayerByPhone(TargetData.number)
    if Target ~= nil then
        TriggerClientEvent('kg-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

RegisterNetEvent('kg-phone:server:BillingEmail', function(data, paid)
    for _, v in pairs(KGCore.Functions.GetPlayers()) do
        local target = KGCore.Functions.GetPlayer(v)
        if target.PlayerData.job.name == data.society then
            if paid then
                local name = '' .. KGCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. KGCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('kg-phone:client:BillingEmail', target.PlayerData.source, data, true, name)
            else
                local name = '' .. KGCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. KGCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('kg-phone:client:BillingEmail', target.PlayerData.source, data, false, name)
            end
        end
    end
end)

RegisterNetEvent('kg-phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('kg-phone:client:UpdateHashtags', -1, Handle, messageData)
end)

RegisterNetEvent('kg-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = KGCore.Functions.GetPlayer(src).citizenid
    KGPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)

RegisterNetEvent('kg-phone:server:DeleteTweet', function(tweetId)
    local Player = KGCore.Functions.GetPlayer(source)
    local delete = false
    local TID = tweetId
    ...
    if delete then
        ...
        TriggerClientEvent('kg-phone:client:UpdateTweets', -1, TWData, nil, true)
    end
end)

RegisterNetEvent('kg-phone:server:UpdateTweets', function(NewTweets, TweetData)
    local src = source
    if Config.Linux then
        ...
        TriggerClientEvent('kg-phone:client:UpdateTweets', -1, src, NewTweets, TweetData, false)
    else
        MySQL.insert('INSERT INTO phone_tweets (citizenid, firstName, lastName, message, date, url, picture, tweetid) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
            TweetData.citizenid,
            TweetData.firstName,
...
RegisterNetEvent('kg-phone:server:TransferMoney', function(iban, amount)
    local src = source
    local sender = KGCore.Functions.GetPlayer(src)
    ...
    if result[1] ~= nil then
        ...
        if reciever ~= nil then
            ...
            if PhoneItem ~= nil then
                TriggerClientEvent('kg-phone:client:TransferMoney', reciever.PlayerData.source, amount,
        ...
        TriggerClientEvent('KGCore:Notify', src, "This account number doesn't exist!", 'error')
    end
end)

RegisterNetEvent('kg-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, _)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    MySQL.update(
...
RegisterNetEvent('kg-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
...
RegisterNetEvent('kg-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
...
RegisterNetEvent('kg-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, _)
    local src = source
    local SenderData = KGCore.Functions.GetPlayer(src)
    local query = '%' .. ChatNumber .. '%'
    local Player = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
    if Player[1] ~= nil then
        local TargetData = KGCore.Functions.GetPlayerByCitizenId(Player[1].citizenid)
        if TargetData ~= nil then
            ...
            if Chat[1] ~= nil then
                ...
                TriggerClientEvent('kg-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, false)
            else
                -- Insert for target
                ...
                -- Send notification & Update messages for target
                TriggerClientEvent('kg-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, true)
            end
        else
            local Chat = MySQL.query.await('SELECT * FROM phone_messages WHERE citizenid = ? AND number = ?', { SenderData.PlayerData.citizenid, ChatNumber })
            if Chat[1] ~= nil then
...
RegisterNetEvent('kg-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = KGCore.Functions.GetPlayer(src)
    local Hour = os.date('%H')
    local Minute = os.date('%M')
    local label = Hour .. ':' .. Minute
    TriggerClientEvent('kg-phone:client:AddRecentCall', src, data, label, type)
    local Trgt = KGCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('kg-phone:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterNetEvent('kg-phone:server:CancelCall', function(ContactData)
    local Ply = KGCore.Functions.GetPlayerByPhone(ContactData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('kg-phone:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('kg-phone:server:AnswerCall', function(CallData)
    local Ply = KGCore.Functions.GetPlayerByPhone(CallData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('kg-phone:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('kg-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
...
RegisterNetEvent('kg-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('kg-phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterNetEvent('kg-phone:server:AddTransaction', function(data)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
...
RegisterNetEvent('kg-phone:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    ...
    -- TriggerClientEvent('kg-phone:RefreshPhone', src)
end)

RegisterNetEvent('kg-phone:server:RemoveInstallation', function(App)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
...
RegisterNetEvent('kg-phone:server:addImageToGallery', function(image)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    MySQL.insert('INSERT INTO phone_gallery (`citizenid`, `image`) VALUES (?, ?)', { Player.PlayerData.citizenid, image })
end)

RegisterNetEvent('kg-phone:server:getImageFromGallery', function()
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    local images = MySQL.query.await('SELECT * FROM phone_gallery WHERE citizenid = ? ORDER BY `date` DESC', { Player.PlayerData.citizenid })
    TriggerClientEvent('kg-phone:refreshImages', src, images)
end)

RegisterNetEvent('kg-phone:server:RemoveImageFromGallery', function(data)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
...
RegisterNetEvent('kg-phone:server:sendPing', function(data)
    local src = source
    if src == data then
        TriggerClientEvent('KGCore:Notify', src, 'You cannot ping yourself', 'error')
    end
end)

-- Command

KGCore.Commands.Add('setmetadata', 'Set Player Metadata (God Only)', {}, false, function(source, args)
    local Player = KGCore.Functions.GetPlayer(source)
    if args[1] then
...
KGCore.Commands.Add('bill', 'Bill A Player', { { name = 'id', help = 'Player ID' }, { name = 'amount', help = 'Fine Amount' } }, false, function(source, args)
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('KGCore:Notify', source, 'Invoice Successfully Sent', 'success')
                    TriggerClientEvent('KGCore:Notify', billed.PlayerData.source, 'New Invoice Received')
                else
                    TriggerClientEvent('KGCore:Notify', source, 'Must Be A Valid Amount Above 0', 'error')
                end
            else
                TriggerClientEvent('KGCore:Notify', source, 'You Cannot Bill Yourself', 'error')
            end
        else
            TriggerClientEvent('KGCore:Notify', source, 'Player Not Online', 'error')
        end
    else
        TriggerClientEvent('KGCore:Notify', source, 'No Access', 'error')
    end
end)
