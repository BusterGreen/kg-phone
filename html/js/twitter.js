...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('https://kg-phone/ClearMentions');
        }
...
$(document).on('click', '.twitter-new-tweet', function(e){
    ...
    KG.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
});
...
KG.Phone.Notifications.LoadTweets = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            var TwtMessage = KG.Phone.Functions.FormatTwitterMessage(clean);
            ...
            // if (Tweet.citizenid === KG.Phone.Data.PlayerData.citizenid){
...
$(document).on('click','#twt-delete-click',function(e){
    ...
    $.post('https://kg-phone/DeleteTweet', JSON.stringify({id: source}))
})
...
KG.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    $('#tweet-new-url').val("");
    if (Tweets.length > 0) {
...
KG.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('https://kg-phone/PostNewTweet', JSON.stringify({
            Message: TweetMessage,
            Date: CurrentDate,
            Picture: KG.Phone.Data.MetaData.profilepicture,
            url: imageURL
        }), function(Tweets){
            KG.Phone.Notifications.LoadTweets(Tweets);
        });
        ...
        KG.Phone.Animations.TopSlideUp(".twitter-new-tweet-tab", 450, -120);
    } else {
        KG.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Fill a message!", "#1DA1F2");
    };
    $('#tweet-new-url').val("");
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('https://kg-phone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            KG.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
function CopyMentionTag(elem) {
    ...
    KG.Phone.Notifications.Add("fab fa-twitter", "Twitter", $(elem).data('mentiontag')+ " copied!", "rgb(27, 149, 224)", 1250);
    document.execCommand("copy");
    $temp.remove();
}

KG.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
...
KG.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    KG.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
