...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Wallet: "+CryptoData.Portfolio+" Kgit('s)");
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (KG.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('https://kg-phone/BuyCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    KG.Phone.Data.PlayerData.money.bank = parseInt(KG.Phone.Data.PlayerData.money.bank) - parseInt(Price);
                    KG.Phone.Notifications.Add("fas fa-university", "KGank", "&#36; "+Price+",- has been withdrawn from your balance!", "#badc58", 2500);
                } else {
                    KG.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough money..", "#badc58", 1500);
                }
            });
        } else {
            KG.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough money..", "#badc58", 1500);
        ...
        KG.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Fill out all fields!", "#badc58", 1500);
    }
});
...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://kg-phone/SellCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    KG.Phone.Data.PlayerData.money.bank = parseInt(KG.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    KG.Phone.Notifications.Add("fas fa-university", "KGank", "&#36; "+Price+",- has been added to your balance!", "#badc58", 2500);
                } else {
                    KG.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Kgits..", "#badc58", 1500);
                }
            });
        } else {
            KG.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Kgits..", "#badc58", 1500);
    ...
    if ((Coins !== "") && (WalletId !== "")) {
        if (CryptoData.Portfolio >= Coins) {
            if (WalletId !== CryptoData.WalletId) {
                $.post('https://kg-phone/TransferCrypto', JSON.stringify({
                    Coins: Coins,
                    WalletId: WalletId,
                }), function(CryptoData){
                    if (CryptoData == "notenough") {
                        ...
                        KG.Phone.Notifications.Add("fas fa-university", "Crypto", "this Wallet-ID doesn't exist!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        KG.Phone.Notifications.Add("fas fa-university", "Crypto", "You transferred "+Coins+",- to "+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                KG.Phone.Notifications.Add("fas fa-university", "Crypto", "You can't transfer to yourself..", "#badc58", 2500);
            }
        } else {
            KG.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Kgits..", "#badc58", 1500);
        }
