...
$(document).on('click', '.mail', function(e){
    ...
    KG.Phone.Functions.SetupMail(MailData);
...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://kg-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://kg-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
KG.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(KG.Phone.Data.PlayerData.charinfo.firstname+"."+KG.Phone.Data.PlayerData.charinfo.lastname+"@kgcore.com");
...
KG.Phone.Functions.SetupMail = function(MailData) {
...
$(document).on('click','#new-advert-photo',function(e){
    e.preventDefault();
    $.post('https://kg-phone/TakePhoto',function(url){
        if(url){
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        if (!picture){
            $.post('https://kg-phone/PostAdvert', JSON.stringify({
                message: Advert,
                url: null
            }));
        }else {
        ...
        KG.Phone.Notifications.Add("fas fa-ad", "Advertisement", "You can\'t post an empty ad!", "#ff8f1a", 2000);
    }
});

KG.Phone.Functions.RefreshAdverts = function(Adverts) {
    $("#advert-header-name").html("@"+KG.Phone.Data.PlayerData.charinfo.firstname+""+KG.Phone.Data.PlayerData.charinfo.lastname+" | "+KG.Phone.Data.PlayerData.charinfo.phone);
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $.each(Adverts, function(i, advert){
            ...
            if (advert.number === KG.Phone.Data.PlayerData.charinfo.phone){
...
$(document).on('click','#adv-delete',function(e){
    e.preventDefault();
    $.post('https://kg-phone/DeleteAdvert', function(){
        setTimeout(function(){
            KG.Phone.Notifications.Add("fas fa-ad", "Advertisement", "The ad was deleted", "#ff8f1a", 2000);
        },400)
    });
})
