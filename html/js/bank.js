...
$(document).on('click', '.bank-app-account', function(e){
    ...
    KG.Phone.Notifications.Add("fas fa-university", "KGank", "Account number. copied!", "#badc58", 1750);
});
...
KG.Phone.Functions.DoBankOpen = function() {
    KG.Phone.Data.PlayerData.money.bank = (KG.Phone.Data.PlayerData.money.bank).toFixed();
    $(".bank-app-account-number").val(KG.Phone.Data.PlayerData.charinfo.account);
    $(".bank-app-account-balance").html("&#36; "+KG.Phone.Data.PlayerData.money.bank);
    $(".bank-app-account-balance").data('balance', KG.Phone.Data.PlayerData.money.bank);

    $(".bank-app-loaded").css({"display":"none", "padding-left":"30vh"});
    $(".bank-app-accounts").css({"left":"30vh"});
    $(".kgank-logo").css({"left": "0vh"});
    $("#kgank-text").css({"opacity":"0.0", "left":"9vh"});
...
$(document).on('click', '.bank-app-account-actions', function(e){
    KG.Phone.Animations.TopSlideDown(".bank-app-transfer", 400, 0);
});

$(document).on('click', '#cancel-transfer', function(e){
    e.preventDefault();

    KG.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
});
...
$(document).on('click', '#accept-transfer', function(e){
    ...
    if (iban != "" && amount != "") {
            $.post('https://kg-phone/CanTransferMoney', JSON.stringify({
                sendTo: iban,
                amountOf: amount,
            }), function(data){
                if (data.TransferedMoney) {
                    ...
                    KG.Phone.Notifications.Add("fas fa-university", "KGank", "You have transfered &#36; "+amount+"!", "#badc58", 1500);
                } else {
                    KG.Phone.Notifications.Add("fas fa-university", "KGank", "You don't have enough balance!", "#badc58", 1500);
                }
                KG.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        KG.Phone.Notifications.Add("fas fa-university", "KGank", "Fill out all fields!", "#badc58", 1750);
    }
});
...
$(document).on('click', '.pay-invoice', function(event){
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://kg-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                KG.Phone.Notifications.Add("fas fa-university", "KGank", "You have paid &#36;"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('https://kg-phone/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        society: InvoiceData.society,
        invoiceId: InvoiceData.id,
...
KG.Phone.Functions.LoadBankInvoices = function(invoices) {
    if (invoices !== null) {
...
KG.Phone.Functions.LoadContactsWithNumber = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            var ContactColor = KG.Phone.ContactColors[RandomNumber];
...
$(document).on('click', '.bank-app-my-contacts-list-back', function(e){
    ...
    KG.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});

$(document).on('click', '.bank-transfer-mycontacts-icon', function(e){
    e.preventDefault();

    KG.Phone.Animations.TopSlideDown(".bank-app-my-contacts", 400, 0);
});
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        KG.Phone.Notifications.Add("fas fa-university", "KGank", "There is no bank account attached to this number!", "#badc58", 2500);
    }
    KG.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});