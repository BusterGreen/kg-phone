...
SetupMeosHome = function() {
    $("#meos-app-name").html("Welcome " + KG.Phone.Data.PlayerData.charinfo.firstname + " " + KG.Phone.Data.PlayerData.charinfo.lastname);
}
...
$(document).on('click', '.house-adress-location', function(e){
    ...
    $.post('https://kg-phone/SetGPSLocation', JSON.stringify({
...
$(document).on('click', '.appartment-adress-location', function(e){
    ...
    $.post('https://kg-phone/SetApartmentLocation', JSON.stringify({
...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    KG.Phone.Notifications.Add("fas fa-university", "MDT", "House number copied!", "#badc58", 1750);
...
$(document).on('click', '.confirm-search-person-test', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://kg-phone/FetchSearchResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
                ...
                KG.Phone.Notifications.Add("politie", "MDT", "There are no search results!");
                $(".person-search-results").html("");
            }
        });
    } else {
...
$(document).on('click', '.confirm-search-person-house', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://kg-phone/FetchPlayerHouses', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
...
$(document).on('click', '.confirm-search-vehicle', function(e){
    ...
    if (SearchName !== "") {
        $.post('https://kg-phone/FetchVehicleResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".vehicle-search-results").html("");
...
$(document).on('click', '.scan-search-vehicle', function(e){
    e.preventDefault();
    $.post('https://kg-phone/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            $(".vehicle-search-results").html("");
            var APK = "Yes";
            if (!vehicle.status) {
            ...
            KG.Phone.Notifications.Add("politie", "MDT", "No vehicle nearby!");
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData != undefined){
        if (alertData.coords != undefined && alertData.coords != null) {
            $.post('https://kg-phone/SetAlertWaypoint', JSON.stringify({
                alert: alertData,
            }));
        } else {
            KG.Phone.Notifications.Add("politie", "MDT", "This alert doesn't have a GPS location!");
        }
    }else {
        KG.Phone.Notifications.Add("politie", "MDT", "There are no alerts available.");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    ...
    KG.Phone.Notifications.Add("politie", "MDT", "All alerts have been deleted!");
});
