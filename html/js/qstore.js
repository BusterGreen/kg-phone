...
SetupAppstore = function(data) {
    ...
    $.each(data.StoreApps, function(i, app){
        if (data.PhoneData.InstalledApps[i] == null || data.PhoneData.InstalledApps[i] == undefined) {
            if(app.blockedjobs != KG.Phone.Data.PlayerJob.name){
...
$(document).on('click', '.storeapp-remove', function(e){
    ...
    KG.Phone.Data.Applications[AppData.app] = null;

    $.post('https://kg-phone/RemoveApplication', JSON.stringify({
        app: AppData.app
    }));
    setTimeout(function(){
...
$(document).on('click', '.download-password-accept', function(e){
    ...
    if (FilledInPassword == TemplatePassword) {
        ...
        $(".download-progressbar-fill").animate({
            ...
            $.post('https://kg-phone/InstallApplication', JSON.stringify({
                app: CurrentApp,
            }), function(Installed){
                if (Installed) {
                    var applicationSlot = $(".phone-applications").find('[data-appslot="'+Installed.data.slot+'"]');
                    var blockedapp = IsAppJobBlocked(Installed.data.blockedjobs, KG.Phone.Data.PlayerJob.name)
                    if ((!Installed.data.job || Installed.data.job === KG.Phone.Data.PlayerJob.name) && !blockedapp) {
                    ...
                    KG.Phone.Data.Applications[Installed.data.app] = Installed.data;
