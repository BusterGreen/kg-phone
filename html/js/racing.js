...
$(document).on('click', '.racing-race', function(e){
    ...
    var IsRacer = IsInRace(KG.Phone.Data.PlayerData.citizenid, RaceData.RaceData.Racers)
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "The race already started..", "#1DA1F2");
    }
});
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            ...
            var InRace = IsInRace(KG.Phone.Data.PlayerData.citizenid, race.RaceData.Racers);
            var Creator = IsCreator(KG.Phone.Data.PlayerData.citizenid, race);
...
$(document).on('click', '#join-race', function(e){
    ...
    $.post('https://kg-phone/IsInRace', JSON.stringify({}), function(IsInRace){
        if (!IsInRace) {
            $.post('https://kg-phone/RaceDistanceCheck', JSON.stringify({
                RaceId: Data.RaceId,
                Joined: true,
            }), function(InDistance){
                if (InDistance) {
                    $.post('https://kg-phone/IsBusyCheck', JSON.stringify({
                        check: "editor"
                    }), function(IsBusy){
                        if (!IsBusy) {
                            $.post('https://kg-phone/JoinRace', JSON.stringify({
                                ...Data
                            }));
                            ...
                            KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You're in a editor..", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You're already in a race..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#quit-race', function(e){
    ...
    $.post('https://kg-phone/LeaveRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '#start-race', function(e){
    ...
    $.post('https://kg-phone/StartRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('https://kg-phone/GetTrackData', JSON.stringify({
        RaceId: $(this).attr('id')
    }), function(TrackData){
...
$(document).on('click', '#setup-race', function(e){
    ...
    $.post('https://kg-phone/GetRaces', JSON.stringify({}), function(Races){
...
$(document).on('click', '#create-race', function(e){
    e.preventDefault();
    $.post('https://kg-phone/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
            ...
            if (!data.IsBusy) {
                $.post('https://kg-phone/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You're in a race..", "#1DA1F2");
                    }
                });
            } else {
                KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You're already setting up a track..", "#1DA1F2");
            }
        } else {
            KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You don't have rights to make Race Tracks..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        ...
        $.post('https://kg-phone/IsAuthorizedToCreateRaces', JSON.stringify({
            TrackName: TrackName
        }), function(data){
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    $.post('https://kg-phone/StartTrackEditor', JSON.stringify({
                        TrackName: TrackName
                    }));
                    ...
                    KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "This name is not available..", "#1DA1F2");
                }
            } else {
                KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You don't have any rights to create Race Tracks..", "#1DA1F2");
            }
        });
    } else {
        KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have to enter a track name..", "#1DA1F2");
    }
});
...
$(document).on('click', '#setup-race-accept', function(e){
    ...
    $.post('https://kg-phone/HasCreatedRace', JSON.stringify({}), function(HasCreatedRace){
        if (!HasCreatedRace) {
            $.post('https://kg-phone/RaceDistanceCheck', JSON.stringify({
                RaceId: track,
                Joined: false,
                ...
                if (InDistance) {
                    if (track !== undefined || track !== null) {
                        if (laps !== "") {
                            $.post('https://kg-phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    $.post('https://kg-phone/SetupRace', JSON.stringify({
                                        RaceId: track,
                                        AmountOfLaps: laps,
                                    }))
                                    $(".racing-overview").animate({
                                    ...
                                    KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "There can't be any ..", "#1DA1F2");
                                }
                            });
                        } else {
                            KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Fill in an amount of laps..", "#1DA1F2");
                        }
                    } else {
                        KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You haven't selected a track..", "#1DA1F2");
                    }
                }
            })
        } else {
            KG.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You already have a race active..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#leaderboards-race', function(e){
    ...
    $.post('https://kg-phone/GetRacingLeaderboards', JSON.stringify({}), function(Races){
        if (Races !== null) {
