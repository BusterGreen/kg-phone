...
$(document).on('click', '.whatsapp-chat', function(e){
    ...
    KG.Phone.Functions.SetupChatMessages(ChatData);

    $.post('https://kg-phone/ClearAlerts', JSON.stringify({
...
KG.Phone.Functions.GetLastMessage = function(messages) {
...
KG.Phone.Functions.LoadWhatsappChats = function(chats) {
    $(".whatsapp-chats").html("");
    $.each(chats, function(i, chat){
        ...
        var LastMessage = KG.Phone.Functions.GetLastMessage(chat.messages);
...
KG.Phone.Functions.ReloadWhatsappAlerts = function(chats) {
    $.each(chats, function(i, chat){
        if (chat.Unread > 0 && chat.Unread !== undefined && chat.Unread !== null) {
            $(".unread-chat-id-"+i).html(chat.Unread);
            $(".unread-chat-id-"+i).css({"display":"block"});
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        $.post('https://kg-phone/SendMessage', JSON.stringify({
        ...
        KG.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "You can't send a empty message!", "#25D366", 1750);
    }
});
...
KG.Phone.Functions.SetupChatMessages = function(cData, NewChatData) {
    if (cData) {
        OpenedChatData.number = cData.number;

        if (OpenedChatPicture == null) {
            $.post('https://kg-phone/GetProfilePicture', JSON.stringify({
        ...
        $.each(cData.messages, function(i, chat){
            ...
            $.each(cData.messages[i].messages, function(index, message){
                ...
                var Sender = "me";
                if (message.sender !== KG.Phone.Data.PlayerData.citizenid) { Sender = "other"; }
                var MessageElement
...
$(document).on('click', '.whatsapp-shared-location', function(e){
    ...
    $.post('https://kg-phone/SharedLocation', JSON.stringify({
