...
local function findVehFromPlateAndLocate(plate)
    local gameVehicles = KGCore.Functions.GetVehicles()
    for i = 1, #gameVehicles do
        local vehicle = gameVehicles[i]
        if DoesEntityExist(vehicle) then
            if KGCore.Functions.GetPlate(vehicle) == plate then
...
local function LoadPhone()
    Wait(100)
    KGCore.Functions.TriggerCallback('kg-phone:server:GetPhoneData', function(pData)
        PlayerJob = KGCore.Functions.GetPlayerData().job
        PhoneData.PlayerData = KGCore.Functions.GetPlayerData()
...
local function OpenPhone()
    KGCore.Functions.TriggerCallback('kg-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            PhoneData.PlayerData = KGCore.Functions.GetPlayerData()
            ...
            KGCore.Functions.TriggerCallback('kg-garages:server:GetPlayerVehicles', function(vehicles)
                PhoneData.GarageVehicles = vehicles
            end)
        else
            KGCore.Functions.Notify("You don't have a phone", 'error')
        end
    end)
end
...
local function CancelCall()
    TriggerServerEvent('kg-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('kg-phone:server:SetCallState', false)
...
local function CallContact(CallData, AnonymousCall)
    ...
    TriggerServerEvent('kg-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('kg-phone:server:SetCallState', true)
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('kg-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            KGCore.Functions.Notify('Action not available at the moment..', 'error')
        end
    end
end)
...
RegisterNUICallback('ClearRecentAlerts', function(_, cb)
    TriggerServerEvent('kg-phone:server:SetPhoneAlerts', 'phone', 0)
...
RegisterNUICallback('SetBackground', function(data, cb)
    ...
    TriggerServerEvent('kg-phone:server:SaveMetaData', PhoneData.MetaData)
    cb('ok')
end)
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('kg-phone:server:RemoveMail', MailId)
    cb('ok')
end)
...
RegisterNUICallback('AcceptMailButton', function(data, cb)
    ...
    TriggerServerEvent('kg-phone:server:ClearButtonData', data.mailId)
    cb('ok')
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('kg-phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    KGCore.Functions.TriggerCallback('kg-phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('PostAdvert', function(data, cb)
    TriggerServerEvent('kg-phone:server:AddAdvert', data.message, data.url)
    cb('ok')
end)

RegisterNUICallback('DeleteAdvert', function(_, cb)
    TriggerServerEvent('kg-phone:server:DeleteAdvert')
    cb('ok')
end)
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('kg-phone:server:SetPhoneAlerts', 'whatsapp', newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    KGCore.Functions.TriggerCallback('kg-phone:server:PayInvoice', function(CanPay, Invoices)
        if CanPay then PhoneData.Invoices = Invoices end
        cb(CanPay)
    end, society, amount, invoiceId, senderCitizenId)
    TriggerServerEvent('kg-phone:server:BillingEmail', data, true)
end)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    local amount = data.amount
    local invoiceId = data.invoiceId
    KGCore.Functions.TriggerCallback('kg-phone:server:DeclineInvoice', function(_, Invoices)
        PhoneData.Invoices = Invoices
        cb('ok')
    end, society, amount, invoiceId)
    TriggerServerEvent('kg-phone:server:BillingEmail', data, false)
end)
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('kg-phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
    ...
    if #Hashtag <= 3 then
        for i = 2, #Hashtag, 1 do
            ...
            if Handle ~= nil or Handle ~= '' then
                ...
                TriggerServerEvent('kg-phone:server:UpdateHashtags', Handle, TweetMessage)
            end
        end
            ...
            if Handle ~= nil or Handle ~= '' then
                ...
                if (Firstname ~= nil and Firstname ~= '') and (Lastname ~= nil and Lastname ~= '') then
                    if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                        TriggerServerEvent('kg-phone:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                    end
                end
            end
        end
        ...
        TriggerServerEvent('kg-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
    else
        SendNUIMessage({
            action = 'PhoneNotification',
            PhoneNotify = {
...
RegisterNUICallback('DeleteTweet', function(data, cb)
    TriggerServerEvent('kg-phone:server:DeleteTweet', data.id)
    cb('ok')
end)
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    KGCore.Functions.TriggerCallback('kg-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('kg-phone:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('RemoveApplication', function(data, cb)
    TriggerServerEvent('kg-phone:server:RemoveInstallation', data.app)
    cb('ok')
end)

RegisterNUICallback('GetTruckerData', function(_, cb)
    local TruckerMeta = KGCore.Functions.GetPlayerData().metadata['jobrep']['trucker']
    local TierData = exports['kg-trucker']:GetTier(TruckerMeta)
    cb(TierData)
end)
...
RegisterNUICallback('DeleteImage', function(image, cb)
    TriggerServerEvent('kg-phone:server:RemoveImageFromGallery', image)
    Wait(400)
    TriggerServerEvent('kg-phone:server:getImageFromGallery')
    cb(true)
end)
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        KGCore.Functions.Notify('Your vehicle has been marked', 'success')
    else
        KGCore.Functions.Notify('This vehicle cannot be located', 'error')
    end
    cb('ok')
end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('kg-phone:server:RemoveContact', Name, Number)
end)

RegisterNUICallback('GetCryptoData', function(data, cb)
    KGCore.Functions.TriggerCallback('kg-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    KGCore.Functions.TriggerCallback('kg-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    KGCore.Functions.TriggerCallback('kg-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    KGCore.Functions.TriggerCallback('kg-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)
...
RegisterNUICallback('GetAvailableRaces', function(_, cb)
    KGCore.Functions.TriggerCallback('kg-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data, cb)
    TriggerServerEvent('kg-lapraces:server:JoinRace', data.RaceData)
    cb('ok')
end)

RegisterNUICallback('LeaveRace', function(data, cb)
    TriggerServerEvent('kg-lapraces:server:LeaveRace', data.RaceData)
    cb('ok')
end)

RegisterNUICallback('StartRace', function(data, cb)
    TriggerServerEvent('kg-lapraces:server:StartRace', data.RaceData.RaceId)
    cb('ok')
end)

RegisterNUICallback('SetAlertWaypoint', function(data, cb)
    local coords = data.alert.coords
    KGCore.Functions.Notify('GPS Location set: ' .. data.alert.title)
...
RegisterNUICallback('FetchVehicleResults', function(data, cb)
    KGCore.Functions.TriggerCallback('kg-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then
            for k, _ in pairs(result) do
                KGCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('FetchVehicleScan', function(_, cb)
    ...
    KGCore.Functions.TriggerCallback('kg-phone:server:ScanPlate', function(result)
        KGCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
            if KGCore.Shared.Vehicles[vehname] ~= nil then
                result.label = KGCore.Shared.Vehicles[vehname]['name']
            else
...
RegisterNUICallback('GetRaces', function(_, cb)
    KGCore.Functions.TriggerCallback('kg-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    KGCore.Functions.TriggerCallback('kg-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('kg-lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
    cb('ok')
end)

RegisterNUICallback('HasCreatedRace', function(_, cb)
    KGCore.Functions.TriggerCallback('kg-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(_, cb)
    local InRace = exports['kg-lapraces']:IsInRace()
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    KGCore.Functions.TriggerCallback('kg-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        data = {
            IsAuthorized = IsAuthorized,
            IsBusy = exports['kg-lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('kg-lapraces:server:CreateLapRace', data.TrackName)
    cb('ok')
end)

RegisterNUICallback('GetRacingLeaderboards', function(_, cb)
    KGCore.Functions.TriggerCallback('kg-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    KGCore.Functions.TriggerCallback('kg-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('kg-lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
            KGCore.Functions.Notify('You\'re too far away from the race. GPS has been set to the race.', 'error', 5000)
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == 'editor' then
        cb(exports['kg-lapraces']:IsInEditor())
    else
        cb(exports['kg-lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(_, cb)
    KGCore.Functions.TriggerCallback('kg-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(_, cb)
    KGCore.Functions.TriggerCallback('kg-phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)

RegisterNUICallback('GetPlayerKeys', function(_, cb)
    KGCore.Functions.TriggerCallback('kg-phone:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    KGCore.Functions.Notify('GPS has been set to ' .. data.HouseData.HouseData.adress .. '!', 'success')
    cb('ok')
end)

RegisterNUICallback('RemoveKeyholder', function(data, cb)
    TriggerServerEvent('kg-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    KGCore.Functions.TriggerCallback('kg-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
    end, TransferedCid, data.HouseData)
end)

RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    KGCore.Functions.TriggerCallback('kg-phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    KGCore.Functions.Notify('GPS has been set!', 'success')
    cb('ok')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(_, cb)
    KGCore.Functions.TriggerCallback('kg-phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('ClearMentions', function(_, cb)
    ...
    TriggerServerEvent('kg-phone:server:SetPhoneAlerts', 'twitter', 0)
    SendNUIMessage({ action = 'RefreshAppAlerts', AppData = Config.PhoneApplications })
    cb('ok')
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data, cb)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('kg-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = 'RefreshAppAlerts', AppData = Config.PhoneApplications })
        cb('ok')
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    if tonumber(PhoneData.PlayerData.money.bank) >= data.amount then
        ...
        TriggerServerEvent('kg-phone:server:TransferMoney', data.iban, data.amount)
        local cbdata = {
            CanTransfer = true,
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        KGCore.Functions.TriggerCallback('kg-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('GetWhatsappChats', function(_, cb)
    KGCore.Functions.TriggerCallback('kg-phone:server:GetContactPictures', function(Chats)
        cb(Chats)
    end, PhoneData.Chats)
end)

RegisterNUICallback('CallContact', function(data, cb)
    KGCore.Functions.TriggerCallback('kg-phone:server:GetCallState', function(CanCall, IsOnline, _)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('kg-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('kg-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    KGCore.Functions.TriggerCallback('kg-phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = 'UpdateChat',
            chatData = Chat,
...
RegisterNUICallback('TakePhoto', function(_, cb)
    ...
    while takePhoto do
        ...
        elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
            KGCore.Functions.TriggerCallback('kg-phone:server:GetWebhook', function(hook)
                if not hook then
                    KGCore.Functions.Notify('Camera not setup', 'error')
                    return
                end
                exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), 'files[]', function(data)
                    ...
                    DestroyMobilePhone()
                    CellCamActivate(false, false)
                    TriggerServerEvent('kg-phone:server:addImageToGallery', image.attachments[1].proxy_url)
                    Wait(400)
                    TriggerServerEvent('kg-phone:server:getImageFromGallery')
...
RegisterCommand('ping', function(_, args)
    if not args[1] then
        KGCore.Functions.Notify('You need to input a Player ID', 'error')
    else
        TriggerServerEvent('kg-phone:server:sendPing', args[1])
    end
end, false)
...
RegisterNetEvent('kg-phone:client:TransferMoney', function(amount, newmoney)
    PhoneData.PlayerData.money.bank = newmoney
    SendNUIMessage({ action = 'PhoneNotification', PhoneNotify = { title = 'KGank', text = '&#36;' .. amount .. ' has been added to your account!', icon = 'fas fa-university', color = '#8c7ae6', }, })
    SendNUIMessage({ action = 'UpdateBank', NewBalance = PhoneData.PlayerData.money.bank })
end)

-- RegisterNetEvent('kg-phone:client:UpdateTweetsDel', function(source, Tweets)
...
RegisterNetEvent('kg-phone:client:UpdateTweets', function(src, Tweets, NewTweetData, delete)
    PhoneData.Tweets = Tweets
    local MyPlayerId = PhoneData.PlayerData.source
...
RegisterNetEvent('kg-phone:client:RaceNotify', function(message)
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
            title = 'Racing',
...
RegisterNetEvent('kg-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('kg-phone:server:SetPhoneAlerts', 'phone')
...
RegisterNetEvent('kg-phone-new:client:BankNotify', function(text)
    SendNUIMessage({
        action = 'PhoneNotification',
        NotifyData = {
            title = 'Bank',
...
RegisterNetEvent('kg-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
            title = 'Mail',
    ...
    TriggerServerEvent('kg-phone:server:SetPhoneAlerts', 'mail')
end)

RegisterNetEvent('kg-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = 'UpdateMails',
        Mails = NewMails
    })
...
RegisterNetEvent('kg-phone:client:UpdateAdvertsDel', function(Adverts)
...
RegisterNetEvent('kg-phone:client:UpdateAdverts', function(Adverts, LastAd)
    PhoneData.Adverts = Adverts
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
...
RegisterNetEvent('kg-phone:client:BillingEmail', function(data, paid, name)
    if paid then
...
RegisterNetEvent('kg-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == 'ongoing' then
        SendNUIMessage({
...
RegisterNetEvent('kg-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('kg-phone:server:AddRecentCall', 'missed', CallData)
                    break
                end
                Wait(Config.RepeatTimeout)
            else
...
RegisterNetEvent('kg-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
        ...
        if PhoneData.isOpen then
            ...
            TriggerServerEvent('kg-phone:server:SetPhoneAlerts', 'whatsapp')
        end
    else
...
RegisterNetEvent('kg-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            action = 'PhoneNotification',
            PhoneNotify = {
...
RegisterNetEvent('kg-phone:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
...
RegisterNetEvent('kg-phone:client:AddTransaction', function(_, _, Message, Title)
    local Data = {
    ...
    TriggerServerEvent('kg-phone:server:AddTransaction', Data)
end)

RegisterNetEvent('kg-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications['phone'].Alerts = Config.PhoneApplications['phone'].Alerts + 1
    TriggerServerEvent('kg-phone:server:SetPhoneAlerts', 'phone', Config.PhoneApplications['phone'].Alerts)
end)

RegisterNetEvent('kg-phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNetEvent('kg-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == 'incoming' or PhoneData.CallData.CallType == 'outgoing') and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = 'ongoing'
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNetEvent('kg-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = KGCore.Functions.GetPlayerData().job
...
RegisterNetEvent('kg-phone:client:GiveContactDetails', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('kg-phone:server:GiveContactDetails', PlayerId)
    else
        KGCore.Functions.Notify('No one nearby!', 'error')
    end
end)

RegisterNetEvent('kg-phone:client:UpdateLapraces', function()
    SendNUIMessage({
        action = 'UpdateRacingApp',
    })
end)

RegisterNetEvent('kg-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNetEvent('kg-phone:refreshImages', function(images)
    PhoneData.Images = images
end)

RegisterNetEvent('kg-phone:client:CustomNotification', function(title, text, icon, color, timeout) -- Send a PhoneNotification to the phone from anywhere
    SendNUIMessage({
        action = 'PhoneNotification',
        PhoneNotify = {
            title = title,
